...
def check_dependencies() -> bool:
    ...
    if not Path(OPENSTEGO_JAR).is_file():
...
def run_exif() -> bool:
    ...
    try:
        shutil.copy2(FILE_WM_VISIBLE, FILE_WM_EXIF)
        run_command(
            [
                "exiftool",
                "-overwrite_original",
...
def run_stegano() -> bool:
    ...
    if not Path(FILE_WM_EXIF).is_file():
        print("[WARN] EXIF introuvable, saut steganographie")
        return False
...
def run_signature() -> bool:
    ...
    if not Path(FILE_WM_INVISIBLE).is_file():
        print("[WARN] Filigrane invisible introuvable, saut signature")
        return False
...
def run_blockchain() -> bool:
    ...
    if not Path(FILE_NUM_SIGNED).is_file():
        print("[WARN] Fichier signe introuvable, saut blockchain")
        return False

    try:
        run_command([OTS_BIN, "stamp", str(FILE_NUM_SIGNED)])
    except FileNotFoundError:
...
def show_exif() -> bool:
    ...
    if not Path(FILE_WM_EXIF).is_file():
        print("[INFO] EXIF : aucune info")
        return False

    try:
        run_command(["exiftool", str(FILE_WM_EXIF)])
    except FileNotFoundError:
        print("[ERROR] exiftool est introuvable")
        return False
    except subprocess.CalledProcessError as error:
...
def main(argv: list[str] | None = None) -> int:
    ...
    ok = run_interactive_flow() if MODE_INTERACTIVE else run_action()
